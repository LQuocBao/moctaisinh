class ContactManager {
  ...
  async handleSubscriptionSubmit(form) {
    const nameInput =
      form.querySelector("#subscriptionName") ||
      form.querySelector('input[placeholder*="tên"]');
    const phoneInput =
      form.querySelector("#subscriptionPhone") ||
      form.querySelector('input[placeholder*="điện thoại"]');
    ...
    try {
      ...
      const response = await fetch(
        `${this.API_BASE}/api/contacts/subscription`,
        {
          method: "POST",
          headers: {
      ...
      this.saveToLocalStorage("subscription", {
        name,
        phone,
  ...
  async handleContactSubmit(form) {
    ...
    submitBtn.disabled = true;
    submitBtn.innerHTML =
      '<span class="fa fa-spinner fa-spin"></span> Gửi liên hệ...';
    try {
  ...
  async sendContactWebhook(name, phone, type, email = "", message = "") {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          `❌ Contact webhook failed for ${type}:`,
          response.status
        );
      }
    } catch (error) {
